    ...
    public function delete(): void
    {
        ...
        try {
            ...
            if (!empty($appIds)) {
                ...
                try { $pdo->prepare("DELETE FROM application_stages WHERE application_id IN ($placeholders)")->execute($appIds); } catch (\Throwable $e) {}
                try { $pdo->prepare("DELETE FROM application_status_history WHERE application_id IN ($placeholders)")->execute($appIds); } catch (\Throwable $e) {}
                try { $pdo->prepare("DELETE FROM application_assignments WHERE application_id IN ($placeholders)")->execute($appIds); } catch (\Throwable $e) {}
                try { $pdo->prepare("DELETE FROM application_notes WHERE application_id IN ($placeholders)")->execute($appIds); } catch (\Throwable $e) {}
                try { $pdo->prepare("DELETE FROM application_tasks WHERE application_id IN ($placeholders)")->execute($appIds); } catch (\Throwable $e) {}
                try { $pdo->prepare("DELETE FROM application_returns WHERE application_id IN ($placeholders)")->execute($appIds); } catch (\Throwable $e) {}
                try { $pdo->prepare("DELETE FROM visa_approvals WHERE application_id IN ($placeholders)")->execute($appIds); } catch (\Throwable $e) {}
                try { $pdo->prepare("DELETE FROM visa_rejections WHERE application_id IN ($placeholders)")->execute($appIds); } catch (\Throwable $e) {}
            ...
            try { $pdo->prepare("DELETE FROM refunds WHERE customer_id = ?")->execute([$id]); } catch (\Throwable $e) {}
            try { $pdo->prepare("DELETE FROM invoices WHERE customer_id = ?")->execute([$id]); } catch (\Throwable $e) {}
            try { $pdo->prepare("DELETE FROM payments WHERE customer_id = ?")->execute([$id]); } catch (\Throwable $e) {}
            ...
            try { $pdo->prepare("DELETE FROM customer_passports WHERE customer_id = ?")->execute([$id]); } catch (\Throwable $e) {}
            try { $pdo->prepare("DELETE FROM customer_national_ids WHERE customer_id = ?")->execute([$id]); } catch (\Throwable $e) {}
            try { $pdo->prepare("DELETE FROM customer_residences WHERE customer_id = ?")->execute([$id]); } catch (\Throwable $e) {}
            try { $pdo->prepare("DELETE FROM customer_family WHERE customer_id = ?")->execute([$id]); } catch (\Throwable $e) {}

            // Applications
            try { $pdo->prepare("DELETE FROM applications WHERE customer_id = ?")->execute([$id]); } catch (\Throwable $e) {}
